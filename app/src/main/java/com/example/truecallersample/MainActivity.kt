...
class MainActivity : AppCompatActivity() {
    ...
    private fun checkAndRequestPermissions(): Boolean {
        ...
        val listPermissionsNeeded = arrayOf<String>()

        if (readPhoneState != PackageManager.PERMISSION_GRANTED) {
            listPermissionsNeeded[0] = Manifest.permission.READ_PHONE_STATE
        }

        if (readCallLog != PackageManager.PERMISSION_GRANTED) {
            listPermissionsNeeded[1] = Manifest.permission.READ_CALL_LOG
        }
